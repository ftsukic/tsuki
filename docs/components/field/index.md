...
`labelWidth`、`labelAlign` 只是在具体适配器调用 Cell 时生成 Cell styles，不会创建隐藏的 Field 布局层。`vertical`、`center`、分割线、箭头和外围布局均由 Cell 负责。
...
`inputStyle` 和 `inputStyles` 只作用于 embedded Input；`styles` 只作用于 `control`、`feedback`、`description` 和 `error`；`cellStyles` 作用于 Cell。ref 为 Input 的 `TextInputInstance`。
...
`styles` 只提供表单区域语义插槽：`control`、`feedback`、`description` 和 `error`。Cell 区域使用 `cellStyles`，Input 使用 `inputStyle`/`inputStyles`，Picker 使用 `pickerStyle`/`pickerStyles`，三者边界互不重叠。
