import type { ColorMapToken } from '../../interface/maps/colors'
...
/**
 * Mirrors antd v6 `components/theme/themes/shared/genColorMapToken.ts`.
 *
 * This is pure color-token derivation. It does not depend on CSS-in-JS or DOM
 * runtime, so the same Map Token can be consumed by React Native.
 */
export default function genColorMapToken(
  ...
  const colorLinkBase = seed.colorLink || seed.colorInfo
